...
export type Checklist={source:string;parts:{n:number;title:string}[];zones:Zone[];markers:Record<string,{zone:string;floor?:string}>};
...
export function ChecklistView({markers,checklist,done,toggleDone,onShow,detail}:{markers:Marker[];checklist:Checklist;done:number[];toggleDone:(uid:number)=>void;onShow:(m:Marker)=>void;detail:(m:Marker)=>string|null}){
 ...
 return <div className="listview">
  ...
  <div className="list-body">
   ...
   <p className="list-source">Area order follows the <a href={checklist.source} target="_blank" rel="noreferrer">Bulbapedia walkthrough</a>.</p>
  </div>
 </div>;
}
...
export function PokedexView({dex,byId,done,onShow}:{dex:Dex;byId:Map<string,Marker>;done:number[];onShow:(m:Marker)=>void}){
 ...
 useEffect(()=>{try{setManual(JSON.parse(localStorage.getItem('ruta151-dex')||'[]'))}catch{}},[]);
 const toggle=(n:number)=>setManual(old=>{const next=old.includes(n)?old.filter(x=>x!==n):[...old,n];try{localStorage.setItem('ruta151-dex',JSON.stringify(next))}catch{}return next});
 ...
 return <div className="listview">
  ...
  <div className="list-body dex">
   {shown.map(s=>{const c=caught(s),isOpen=openN===s.n;
    const where=s.get==='found'?s.found.slice(0,2).map(f=>`${f.zone} · ${f.how}`).join(' / ')+(s.found.length>2?` and ${s.found.length-2} more`:''):s.get==='evo'&&s.from?`Evolves from ${names.get(s.from.n)}${s.from.method?` (${s.from.method})`:''}`:'Not available in Yellow';
    return <div key={s.n} className={`dex-row ${c?'done':''} ${s.get==='none'?'unavailable':''}`}>
     ...
     {isOpen&&<div className="dex-more">
      ...
      {s.get==='none'&&!s.note&&<p>Not found in Pokémon Yellow: trade it over from Red or Blue.</p>}
     </div>}
    </div>})}
