...
export default function Home(){
 ...
 {selected&&<div className="modal-backdrop" role="presentation" onClick={e=>{if(e.target===e.currentTarget)setSelected(null)}}><dialog open className="drawer" aria-modal="true" aria-label={selected.name}><button className="close" onClick={()=>setSelected(null)} aria-label="Close"><X/></button><small>{selected.encounter?`${selected.category} · ${selected.encounter.zone}`:selected.category}</small><div className="title-row"><Figure m={selected}/><h2>{selected.name}</h2></div>{selected.encounter?<div className="local-encounter">{selected.encounter.sprite?<img src={selected.encounter.sprite} alt=""/>:<Figure m={selected}/>}<div><b>{levels(selected.encounter)}</b><span>Up to {selected.encounter.chance}% encounter rate</span><em>{selected.encounter.methods.join(' · ')}</em></div></div>:<><p><MapPin/>{selected.location||'Location on the map'}</p>{selected.detail&&<p><Info/>{selected.detail}</p>}</>}{!game.untracked.includes(selected.category)&&<button className={`complete ${done.includes(selected.uid)?'checked':''}`} onClick={()=>toggleDone(selected.uid)}>{done.includes(selected.uid)?<><Check/>Caught / completed</>:<>Mark as completed</>}</button>}{!selected.encounter&&<div className="details"><span>ID</span><b>#{selected.uid}</b><span>Map</span><b>{areaName(selected.area)}</b></div>}</dialog></div>}
